...
public interface Pacs008ToPacs009Mapper {
    ...
    default void afterTransactionMap(CreditTransferTransactionInformation2 src, @MappingTarget CreditTransferTransactionInformation3 tgt) {
        ...
        // copy commonly-missing identifiers if present on the source (pmtId or top-level)
        try {
            Object pmt = null;
            try { java.lang.reflect.Method m = src.getClass().getMethod("getPmtId"); pmt = m.invoke(src); } catch (NoSuchMethodException ns) { pmt = null; }
            if (pmt != null) {
                // attempt copying a set of common id fields from PmtId -> target
                String[] oneToOne = new String[] {
                    "AcctSvcrTxId","OthrTxId","SctiesSttlmTxId","IntraPosMvmntId","IntraBalMvmntId",
                    "MktInfrstrctrTxId","PoolId","CmonId","TradId"
                };
                for (String fld : oneToOne) {
                    try { copySimpleProperty(pmt, "get"+fld, tgt, "set"+fld); } catch (Exception ignore) {}
                }
                // also copy other common tx ids (if implemented on pmt)
                try { copySimpleProperty(pmt, "getAcctSvcrTxId", tgt, "setAcctSvcrTxId"); } catch (Exception ignore) {}
                try { copySimpleProperty(pmt, "getOthrTxId", tgt, "setOthrTxId"); } catch (Exception ignore) {}
            }
        } catch (Exception _e) { /* ignore */ }
    }

    // conservative account mapper with small IBAN heuristic: used by MapStruct for CashAccount7 -> CashAccount7
    default CashAccount7 cashAccount7ToCashAccount7(CashAccount7 src) {
        if (src == null) return null;
        CashAccount7 tgt = new CashAccount7();
        // copy the simple id object if possible
        try { copySimpleProperty(src, "getId", tgt, "setId"); } catch (Exception ignore) {}

        // IBAN heuristic: if source id contains an Othr -> Id that looks like an IBAN, attempt to set an IBAN field on the target id choice
        try {
            java.lang.reflect.Method getId = src.getClass().getMethod("getId");
            Object srcId = getId.invoke(src);
            if (srcId != null) {
                // try to get Othr -> Id
                try {
                    java.lang.reflect.Method getOthr = null;
                    try { getOthr = srcId.getClass().getMethod("getOthr"); } catch (NoSuchMethodException ns) { getOthr = null; }
                    Object oth = null;
                    if (getOthr != null) oth = getOthr.invoke(srcId);
                    if (oth == null) {
                        // some models expose Othr as a list
                        try { java.lang.reflect.Method getOthrList = srcId.getClass().getMethod("getOthr"); Object maybe = getOthrList.invoke(srcId); if (maybe instanceof java.util.List) {
                                java.util.List<?> l = (java.util.List<?>) maybe; if (!l.isEmpty()) oth = l.get(0);
                            }
                        } catch (Exception ignore) {}
                    }
                    if (oth != null) {
                        try {
                            java.lang.reflect.Method getIdVal = oth.getClass().getMethod("getId");
                            Object idVal = getIdVal.invoke(oth);
                            if (idVal != null && idVal.toString().matches("^[A-Z]{2}[0-9A-Z]{13,30}$")) {
                                String iban = idVal.toString();
                                // find target id object and attempt to set IBAN property if it exists
                                java.lang.reflect.Method tgtGetId = tgt.getClass().getMethod("getId");
                                Object tgtId = tgtGetId.invoke(tgt);
                                if (tgtId != null) {
                                    // search for a setter named setIban / setIBAN / setIbanId etc.
                                    java.lang.reflect.Method setIban = null;
                                    for (java.lang.reflect.Method mm : tgtId.getClass().getMethods()) {
                                        String n = mm.getName().toLowerCase();
                                        if (n.contains("set") && n.contains("iban")) { setIban = mm; break; }
                                    }
                                    if (setIban != null) {
                                        try { setIban.invoke(tgtId, iban); } catch (Exception ignore) {}
                                    } else {
                                        // fallback: if target id has a setter that accepts the same src id type, set the full id
                                        try { java.lang.reflect.Method setId = tgt.getClass().getMethod("setId", srcId.getClass()); setId.invoke(tgt, srcId); } catch (Exception ignore) {}
                                    }
                                }
                            }
        ...
        // copy other simple properties conservatively
        try { copySimpleProperty(src, "getTp", tgt, "setTp"); } catch (Exception ignore) {}
        try { copySimpleProperty(src, "getCcy", tgt, "setCcy"); } catch (Exception ignore) {}
        try { copySimpleProperty(src, "getNm", tgt, "setNm"); } catch (Exception ignore) {}
        return tgt;
    }
